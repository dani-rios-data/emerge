...
function getCommunityValue(
  ...
  if (feature.properties?.iso) {
    ...
    if (territorioCode) {
      ...
      if (matchByCode) {
        ...
        return parseFloat(matchByCode.OBS_VALUE);
      }
    }
  }
  ...
  if (communityMatch) {
    ...
    return parseFloat(communityMatch.OBS_VALUE);
  }
    ...
    if (normalizedCommunityName === keyNormalized || normalizedCommunityName === valueNormalized) {
      ...
      for (const name of possibleNames) {
        ...
        if (match) {
          ...
          return parseFloat(match.OBS_VALUE);
        }
      }
    }
  }
  ...
  if (matchByContains) {
    ...
    return parseFloat(matchByContains.OBS_VALUE);
  }
    ...
    if (normalizedCommunityName.includes(baseName) || baseName.includes(normalizedCommunityName)) {
      ...
      for (const code of codes) {
        ...
        if (matchBySpecialCode) {
          ...
          return parseFloat(matchBySpecialCode.OBS_VALUE);
        }
      }
      ...
      for (const altName of altNames) {
        ...
        if (matchBySpecialName) {
          ...
          return parseFloat(matchBySpecialName.OBS_VALUE);
        }
      }
    }
  }
...
function getColorForValue(
  ...
  // Si no hay datos, devolver color "sin datos"
  if (filteredData.length === 0) return getSectorPalette(selectedSector).NULL;
  
  // Extraer los valores numéricos de todos los datos filtrados
  const values = filteredData
    .map(item => parseFloat(item.OBS_VALUE))
    .filter(val => !isNaN(val));
  
  // Si no hay valores válidos, devolver color "sin datos"
  if (values.length === 0) return getSectorPalette(selectedSector).NULL;
  ...
  // Calcular los cuartiles para una mejor distribución de colores
  ...
  if (rangeRatio > 15) {
    ...
    const logScale = d3.scaleLog<string>()
      .domain([Math.max(minValue, 0.1), maxValue]) // Evitar logaritmo de 0
...
const ResearchersSpanishRegionsMap: React.FC<ResearchersSpanishRegionsMapProps> = ({ 
  ...
  useEffect(() => {
    ...
    const renderMap = () => {
      ...
      function handleMouseOver(this: d3.BaseType, event: MouseEvent, d: GeoJsonFeature): void {
        ...
        const prevYearValue = getCommunityValue(d, data, (selectedYear - 1).toString(), selectedSector, language);
        ...
        if (value !== null) {
          tooltipContent += `
            ...
            <div class="mb-3">
              ...
              <div class="flex items-center">
                <span class="text-xl font-bold ${value === 0 ? 'text-orange-500' : 'text-blue-700'}">${formatValue(value)}</span>
                ${value === 0 ? `<span class="text-xs ml-2 text-orange-500">${language === 'es' ? 'Sin investigadores' : 'No researchers'}</span>` : ''}
              </div>
          `;
          ...
          tooltipContent += `
            <div class="text-gray-500">${t.noData}</div>
          `;
        }
      ...
      mapGroup.selectAll<SVGPathElement, GeoJsonFeature>('path.mainland')
        ...
        .attr('fill', (d: GeoJsonFeature) => {
          const value = getCommunityValue(d, data, selectedYear.toString(), selectedSector, language);
          const color = getColorForValue(value, selectedSector, data, selectedYear.toString());
          return color;
        })
        .attr('stroke', '#fff')
        .attr('stroke-width', 0.5)
      ...
      // Crear escala de color para la leyenda usando cuartiles
      const validValues = data
        .filter(item => 
          item.TIME_PERIOD === selectedYear.toString() &&
          item.SECTOR_EJECUCION_CODE === (() => {
            switch (selectedSector.toLowerCase()) {
      ...
      if (validValues.length > 0) {
        ...
        // Calcular cuartiles para la leyenda
        const calculateQuantile = (arr: number[], q: number) => {
          const sorted = [...arr].sort((a, b) => a - b);
          const pos = (sorted.length - 1) * q;
          const base = Math.floor(pos);
        ...
        const quartiles = [
          minValue,
          calculateQuantile(validValues, 0.25),
          calculateQuantile(validValues, 0.5),
          calculateQuantile(validValues, 0.75),
          maxValue
        ];
        ...
        // Rectángulos de color con valores
        legendColors.forEach((color, i) => {
          const yPos = i * legendSpacing + 25;
          ...
          if (i < legendColors.length - 1) {
            legendGroup.append('text')
              ...
              .attr('dy', '0.35em')
              .attr('font-size', '10px')
              .text(`${formatValue(quartiles[i])} - ${formatValue(quartiles[i+1])}`);
          } else {
            legendGroup.append('text')
              .attr('x', legendRectSize + 5)
              .attr('y', yPos + legendRectSize / 2)
  ...
  }, [geoJson, data, selectedYear, selectedSector, language, onClick, dataDisplayType, t]);
